...
public class Diseno
{
    ...
    /// <summary>Si es true, el pase de Apple Wallet se genera con el estilo "posterGeneric" (foto a
    /// toda la tarjeta + Acciones destacadas, iOS 27+) en vez del estilo clásico (EventTicket). Aplica
    /// a ambos tipos (sellos y cupón). El pase incluye también el estilo "generic" clásico como
    /// fallback, así los clientes con iOS 26 o anterior lo agregan igual (sin la foto grande).
    /// Off por defecto: es una elección explícita de la empresa.</summary>
    public bool EstiloPoster { get; set; } = false;
