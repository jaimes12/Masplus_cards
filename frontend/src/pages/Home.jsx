...
const PLANS = [
  {
    name: 'Plan Inicio',
    monthly: 149,
    blurb: 'Ideal para cafeterías, barberías y estéticas.',
    features: [
  ...
  },
  {
    ...
    monthly: 249,
    popular: true,
    blurb: 'El equilibrio perfecto entre precio y funciones para hacer crecer tu negocio.',
    features: [
      'Todo lo de Plan Inicio',
      'Estadísticas',
      'Notificaciones',
      'Promociones',
      'Base de clientes',
...
/** Link al WhatsApp del negocio: lo contesta Ricardo (el asesor IA) al instante. */
const WHATSAPP_URL =
  'https://wa.me/5214494250350?text=' +
  encodeURIComponent('Hola! Quiero información de las tarjetas de lealtad digitales 🙌')
...
/** Ícono de WhatsApp en SVG (lucide no trae marcas). */
...
/**
 * Empujones de conversión, pensados para tráfico de anuncios en celular (78% del tráfico):
 * - Botón flotante de WhatsApp (desktop): dudas → Ricardo IA las contesta al momento.
 * - Barra fija inferior en móvil con WhatsApp + "Prueba 14 días gratis", visible tras
 *   pasar el hero — el CTA nunca queda a 4 pantallas de distancia.
 */
function ConversionBoosters() {
  ...
  return (
    <>
      {/* Flotante de WhatsApp — solo desktop; en móvil vive dentro de la barra */}
      <a
        href={WHATSAPP_URL}
        target="_blank"
        rel="noreferrer"
        ...
        className="fixed bottom-6 right-6 z-40 hidden h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-black/20 transition-transform hover:scale-105 sm:flex"
      >
      ...
      <div
        className={`fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/95 p-3 backdrop-blur transition-transform duration-300 sm:hidden ${
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <Link
            to="/empresa/registro"
            className="flex h-11 flex-1 items-center justify-center rounded-full bg-gradient-to-br from-orange-500 to-orange-600 text-sm font-semibold text-white"
          >
...
function Pricing() {
  ...
  return (
    <section id="precios" className="mx-auto max-w-6xl px-6 py-24">
      ...
      {/* La analítica mostró que aquí se pierde el 75%: ven precios y se van sin dar clic.
          Cierre de riesgo cero + salida por WhatsApp para quien no quiere registrarse solo. */}
      <Reveal className="mt-10">
        <div className="rounded-3xl border border-orange-200 bg-gradient-to-br from-orange-50 to-white p-8 text-center">
          <h3 className="text-xl font-semibold">Pruébalo sin arriesgar un peso</h3>
          <p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">
            Empiezas con <span className="font-semibold text-foreground">14 días del Plan Pro gratis, sin tarjeta de
            crédito</span>. Si no te encanta, te quedas con el plan Gratis para siempre — tu tarjeta sigue funcionando.
          </p>
          <div className="mt-5 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link
              to="/empresa/registro"
              className="inline-flex items-center gap-2 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 px-7 py-3 font-semibold text-white shadow-lg shadow-orange-500/25 transition-opacity hover:opacity-90"
            >
              Crear mi tarjeta gratis <ArrowRight className="h-4 w-4" />
            </Link>
            <a
              href={WHATSAPP_URL}
              target="_blank"
            ...
            >
              <WhatsAppIcon className="h-4 w-4 text-[#25D366]" /> ¿Dudas? Escríbenos por WhatsApp
            </a>
          </div>
          <p className="mt-3 text-xs text-muted-foreground">Te contestamos al momento, sin compromiso.</p>
        </div>
      </Reveal>
    </section>
  )
