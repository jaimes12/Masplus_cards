...
export default function Wallet() {
  ...
  const cuponEstadoLabel = tarjeta.cuponRedimido ? 'Canjeado' : cuponVencido ? 'Vencido' : 'Vigente'
        ...
        <div className="animate-card-in relative z-10 w-full max-w-sm overflow-hidden rounded-2xl shadow-lg">
          <div className="relative aspect-[3/4] w-full">
            ...
            <div className="absolute left-4 top-4 flex min-w-0 items-center gap-2">
              {tarjeta.logo ? (
                <img src={tarjeta.logo} alt="" className="h-9 w-9 shrink-0 rounded-full border-2 border-white/80 object-cover" />
              ) : (
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2 border-white/80 bg-white/20 backdrop-blur">
                  <Ticket className="h-4 w-4 text-white" />
                </div>
              )}
              <p className="truncate text-sm font-semibold text-white drop-shadow">{tarjeta.empresaNombre}</p>
            </div>

            <div className="absolute right-4 top-4 rounded-full bg-black/45 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-white backdrop-blur">
              {tarjeta.vencimiento
